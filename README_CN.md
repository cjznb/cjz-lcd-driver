...
A: 在 `inc/lcd_init_seq.h` 中添加新的 `#elif defined(LCD_DRIVER_CHIP_XXX)` 分支，填入控制器的初始化序列，然后在 `inc/lcd_user_config.h` 中选择对应芯片宏即可。可选择性地添加新的寄存器定义头文件（如 `inc/st7735_cmd.h`）。
